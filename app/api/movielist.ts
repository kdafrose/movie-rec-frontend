...
export const createMovielist = async(token:string, title:string, description:string) => {
    const body = {title, description};
    return await apiCall('/newList', 'POST', body, token);
}

export const addInMovielist = async (token:string, movieId:number, movielistId:number) => {
    const body = {movieId:movieId.toString(), movielistId:movielistId.toString()};
    return await apiCall('/addToMovieList', 'POST', body, token)
}

export const deleteMovieInList = async(token:string, movieId:number, movielistId:number) => {
    const body = {movieId:movieId.toString(), movielistId:movielistId.toString()};
    return await apiCall('/removeFromMovielist', 'DELETE', body, token)
}

export const deleteMovielist = async(token:string, movielistId:number) => {
    const body = {movielistId:movielistId.toString()};
    return await apiCall('/removeMovielist', 'DELETE', body, token)
}

export const editMovielist = async(token:string, movielistId:number, title:string, description:string) => {
...
export const getMoviesInMovielist = async(token:string, movielistId:number) => {
