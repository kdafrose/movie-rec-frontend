...
export const createReview = async(token:boolean, movieId:number, rating:number, reviewText:string) => {
...
export const editReview = async(token:boolean, movieId:number, rating:number, reviewText:string) => {
    const body = {movieId:movieId.toString(), numStars:rating.toString(), review:reviewText};
    return await apiCall('/editReview', 'PATCH', body, token);
}

export const deleteReview = async(token:boolean, movieId:number) => {
    const body = {movieId:movieId.toString()};
    return await apiCall('/deleteReview', 'DELETE', body, token);
}

export const getUserReviewsForMovie = async(token:boolean, movieId:number) => {
...
export const getAllReviewsForMovie = async(movieId:number) => {
    const queryParams = {movieId:movieId.toString()};
    return await getApiCall('/getAllReviews', queryParams, false);
}   
